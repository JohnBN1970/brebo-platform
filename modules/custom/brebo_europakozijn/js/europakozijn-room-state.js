(function (Drupal, once) {
  ...
  Drupal.behaviors.breboEuropakozijnRoomState = { attach(context) {
    once('brebo-europakozijn-room-state', '[data-europakozijn-configurator]', context).forEach((root) => {
      ...
      const load=id=>{const r=state.rooms.find(x=>x.id===id);if(!r)return false;state.active=id;roomId.value=id;roomType.value=r.type||'';roomName.value=r.name||'';roomArea.value=r.area_m2||'';ventilation.value=r.ventilation_system||'';frameRoom.value=id;persist();render();form.dispatchEvent(new Event('change',{bubbles:true}));return true;};
      ...
      root.querySelector('[data-ek-save-room]')?.addEventListener('click',save);root.querySelector('[data-ek-new-room]')?.addEventListener('click',fresh);frameRoom.addEventListener('change',()=>{if(frameRoom.value)load(frameRoom.value);});
      render();if(state.active)load(state.active);root.breboEuropakozijnRooms={getRooms:()=>JSON.parse(JSON.stringify(state.rooms)),getRoom:id=>state.rooms.find(r=>r.id===id)||null,loadRoom:load};
    });
  }};
})(Drupal, once);
